// Pure fleet-status derivation, shared by the client (Agents roster, board
// chips, sidebar badges) and the MCP server. Lives in shared/ because both
// sides need the SAME answer to "which agents are waiting/active" — two
// implementations would eventually disagree about the one fact this product
// exists to state. The type import is type-only, so it is erased under every
// module system that compiles this file.
import type { AttentionItem, Project, SessionRef, SourceTool } from "./types.js";
